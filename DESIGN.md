...
The synchronous production wrapper starts only the bundled Browser MCP module and accepts no command, executable, browser path, launch argument, or site directory from the adapter. Every admitted transport call creates one stdio process and one real browser lifecycle, validates the exact server identity, inventory, schemas, metadata, and bounded response, and closes before returning. A default 10-second deadline bounds the operation. On timeout, the official transport terminates the process tree. Tests observe the real MCP worker, Playwright driver, and Chromium descendant PIDs and require them all to be reaped on both success and timeout. Errors collapse to a fixed local failure without copying paths, page content, or Playwright diagnostics into trajectories.
